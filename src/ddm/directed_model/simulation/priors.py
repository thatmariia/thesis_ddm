...
def sample_prior_params(
    ...
    *,
    ...
    Returns a dict with arrays of shape (n_participants,).
    """
    if n_participants <= 0:
    ...
    alpha = rng.uniform(0.8, 2.0, size=n_participants)
    tau = rng.uniform(0.15, 0.6, size=n_participants)
    beta = rng.uniform(0.3, 0.7, size=n_participants)

    if cfg.mu_z_mode == "random":
        mu_z = rng.normal(0.0, 1.0, size=n_participants)
    elif cfg.mu_z_mode == "zero":
        mu_z = np.zeros(n_participants, dtype=float)
    else:
        raise ValueError(f"Unknown mu_z_mode: {cfg.mu_z_mode}")

    if cfg.sigma_z_prior == "abs_normal":
        sigma_z = np.abs(rng.normal(0.5, 0.5, size=n_participants))
    ...
    b = rng.normal(0.0, 1.0, size=n_participants)
    eta = rng.uniform(0.0, 1.0, size=n_participants)
    lambda_param = rng.uniform(-3.0, 3.0, size=n_participants)
